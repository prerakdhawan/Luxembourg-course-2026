...
class Mie_spherical:
    """
    Analytical solutions for scattering from a sphere of a given dispersive/non-dispersive refractive index and radius. 
    All lengths have to be specified in µm.
    ...
    refractive_index_sphere: float or [N_wvl x 1] array 
        refractive index of the sphere. Can be dispersive.

    n_max:  int
        Maximum number of multipoles in the expansion
    ...
    """
    def __init__(self, radius, refractive_index_sphere, n_max, wavelength, refractive_index_background=1):
        self.radius = radius
        self.refractive_index_sphere = refractive_index_sphere
        self.n_max = n_max
        self.wavelength = wavelength 
        self.refractive_index_background = refractive_index_background
        self.m = self.refractive_index_sphere / self.refractive_index_background # relative refractive index (n_particle / n_medium)

    def mie_coeffs(self,n, x):
        """
        Calculate Mie coefficients a_n and b_n for a sphere of radius R at wavelength lam and for the given (n,m) pair.
        ...
        -------
        a_n : complex
            Mie coefficient corresponding to the electric moment for multipole n.
            
        b_n : complex
            Mie coefficient corresponding to the magnetic moment for multipole n.
        """
        mx = self.m * x
        # Riccati-Bessel functions
        psi_n = x * sps.spherical_jn(n, x)
        psi_n_m = mx * sps.spherical_jn(n, mx)
        psi_n_prime = sps.spherical_jn(n, x) + x * sps.spherical_jn(n, x, derivative=True)
        psi_n_m_prime = sps.spherical_jn(n, mx) + mx * sps.spherical_jn(n, mx, derivative=True)
        xi_n = x * (sps.spherical_jn(n, x) + 1j * sps.spherical_yn(n, x))
        xi_n_prime = (sps.spherical_jn(n, x) + 1j * sps.spherical_yn(n, x)) + x * (sps.spherical_jn(n, x, derivative=True) + 1j * sps.spherical_yn(n, x, derivative=True))
        a_n = (self.m * psi_n_m * psi_n_prime - psi_n * psi_n_m_prime) / (self.m * psi_n_m * xi_n_prime - xi_n * psi_n_m_prime)
        b_n = (psi_n_m * psi_n_prime - self.m * psi_n * psi_n_m_prime) / (psi_n_m * xi_n_prime - self.m * xi_n * psi_n_m_prime)
        return a_n, b_n

    def mie_efficiencies(self):
        """
        Calculate Mie efficiencies for extinction, scattering, absorption.
        """
        k = 2 * np.pi / self.wavelength
        x = k * self.radius  # size parameter

        Q_sca = 0.0
        Q_ext = 0.0

        for n in range(1, self.n_max + 1):
            a_n, b_n = self.mie_coeffs(n, x)
    ...
    def polarizabilities(self):
        """
        Calculate electric and magnetic polarizabilities from the Mie coefficients.
        """
        k = 2 * np.pi / self.wavelength
        x = k * self.radius  # size parameter
        a_n, b_n = self.mie_coeffs(self.n_max, x)
        alpha_e = 8.854e-12*(self.refractive_index_background)**2 * 6*np.pi*1j *a_n/ (k**3)
        alpha_m = 8.854e-12*(self.refractive_index_background)**2 * 6*np.pi*1j *b_n / (k**3)
